...
class MyListTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let title = restaurant.value(forKey: "title")as? String {
            cell.textLabel?.text = title
        }
        if let location = restaurant.value(forKey: "local") as? String {
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            context.delete(restaurants[indexPath.row])
            do {
                try context.save()
                print("deleted!")
            } catch let error as NSError {
        ...
        if segue.identifier == "toDetailView" {
            if let destination = segue.destination as? RestaurantViewController {
                if let selectedIndex = self.tableView.indexPathsForSelectedRows?.first?.row {
                    destination.detailRestaurant = restaurants[selectedIndex]
                }
            }
        }
    }
